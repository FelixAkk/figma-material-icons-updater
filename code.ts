...
function buildIconUrl(iconName: string, params: MaterialSymbolsParams): string {
  ...
  // Build filename: iconName_[wght{weight}][grad{grade}][fill1]_{size}px.svg
  // Pattern: always underscore after name, params concatenated, underscore before size
  let filename = iconName;
  let paramString = '';
  
  if (weight !== 400) {
    paramString += `wght${weight}`;
  }
  if (grade !== 0) {
    // Handle negative grade values (gradN25 for -25)
    const gradeParam = grade < 0 ? `gradN${Math.abs(grade)}` : `grad${grade}`;
    paramString += gradeParam;
  }
  if (fill) {
    paramString += 'fill1';
  }
  
  // Add params with underscore if any exist
  if (paramString) {
    filename += `_${paramString}`;
  }
